...
function custom_heading_html( $block_content, $block ) {
	...
	$modified_content = str_replace(
		...
		array(
			'<h1 class="post-heading01"',
			'<h2 class="post-heading02"',
			'<h3 class="post-heading03"',
			'<h4 class="post-heading04"',
			'<h5 class="post-heading05"',
			'<h6 class="post-heading06"',
		),
		$block_content
	);
...
add_filter(
	...
	function ( $block_content, $block ) {
		...
		if ( ( is_admin() || wp_doing_ajax() || defined( 'REST_REQUEST' ) ) && 'core/heading' === $block['blockName'] ) {
...
function enqueue_heading_script() {
	...
	if ( $screen && 'news' === $screen->post_type && 'post' === $screen->base ) {
